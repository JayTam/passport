<script>
import { h } from "vue";

const SpinIcon = Array(12).fill(h("i"));

const CircularIcon = h("svg", { className: "te-loading__circular", viewBox: "25 25 50 50" }, [
  h("circle", { cx: "50", cy: "50", r: "20", fill: "none" }),
]);

export default {
  name: "TeLoading",
  props: {
    type: {
      type: String,
      default: "circular",
      validator(val) {
        return ["circular", "spinner"].indexOf(val) > -1;
      },
    },
    size: {
      type: [String, Number],
      default: "28",
    },
    color: String,
    dark: { type: Boolean, default: false },
  },
  render: function() {
    return h(
      "div",
      {
        className: `te-loading te-loading__${this.type} ${this.dark ? "te-loading__dark" : ""}`,
        style: { color: this.color },
      },
      [
        h(
          "span",
          {
            className: "te-loading__spinner te-loading__spinner--" + this.type,
            style: {
              fontSize: "12px",
              width: this.size + "px",
              height: this.size + "px",
            },
          },
          [this.type === "spinner" ? SpinIcon : CircularIcon]
        ),
      ]
    );
  },
};
</script>

<style lang="less">
@loading-spinner-size: 30px;
@loading-spinner-animation-duration: 0.8s;
@loading-text-font-size: 12px;
@loading-text-color: @color-text;
@loading-color: @color-light;
@padding-xs: 6px;
.te-loading {
  position: relative;
  color: @loading-color;
  font-size: 0;
  vertical-align: middle;

  &__dark {
    color: @color-text;
  }

  &__spinner {
    position: relative;
    display: inline-block;
    width: @loading-spinner-size;
    // compatible for 1.x, users may set width or height in root element
    max-width: 100%;
    height: @loading-spinner-size;
    max-height: 100%;
    vertical-align: middle;
    animation: te-rotate @loading-spinner-animation-duration linear infinite;

    &--spinner {
      animation-timing-function: steps(12);

      i {
        position: absolute;
        top: 0;
        left: 0;
        width: 100%;
        height: 100%;

        &::before {
          display: block;
          width: 2px;
          height: 25%;
          margin: 0 auto;
          background-color: currentColor;
          border-radius: 40%;
          content: " ";
        }
      }
    }

    &--circular {
      animation-duration: 2s;
    }
  }

  &__circular {
    width: 100%;
    height: 100%;
    display: flex;
    justify-content: center;
    align-items: center;

    circle {
      animation: te-circular 1.5s ease-in-out infinite;
      stroke: currentColor;
      stroke-width: 3;
      stroke-linecap: round;
    }
  }

  &__text {
    display: inline-block;
    margin-left: @padding-xs;
    color: @loading-text-color;
    font-size: @loading-text-font-size;
    vertical-align: middle;
  }

  &--vertical {
    display: flex;
    flex-direction: column;
    align-items: center;

    .te-loading__text {
      margin: @padding-xs 0 0;
    }
  }
}

@keyframes te-rotate {
  from {
    transform: rotate(0deg);
  }

  to {
    transform: rotate(360deg);
  }
}

@keyframes te-circular {
  0% {
    stroke-dasharray: 1, 200;
    stroke-dashoffset: 0;
  }

  50% {
    stroke-dasharray: 90, 150;
    stroke-dashoffset: -40;
  }

  100% {
    stroke-dasharray: 90, 150;
    stroke-dashoffset: -120;
  }
}

.generate-spinner(@n, @i: 1) when (@i =< @n) {
  .te-loading__spinner--spinner i:nth-of-type(@{i}) {
    transform: rotate(@i * 30deg);
    opacity: 1 - (0.75 / 12) * (@i - 1);
  }
  .generate-spinner(@n, (@i + 1));
}

.generate-spinner(12);
</style>
