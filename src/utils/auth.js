...
const PAN_DOMAIN =
  process.env.NODE_ENV === "production"
    ? process.env.VUE_APP_PAN_DOMAIN
    : undefined;
...
class Persistence {
  ...
  get() {
    try {
      ...
      const value =
        query[this.key] ?? Cookies.get(this.key, { domain: this.domain });
...
// 第三方登陆行为
export const thirdBehaviorPersistence = new Persistence(
  "third_behavior",
  PAN_DOMAIN
);
// 登陆回跳地址
export const redirectUriPersistence = new Persistence(
  "redirect_uri",
  PAN_DOMAIN
);
// 子应用ID
